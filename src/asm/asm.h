        ...
        ast->dfs([&](AstNode& n) {
            ...
            if (n.operator_type == kCONST) {
                ...
                if (n.value_type == vInts) {
                    //auto q = 
		    new (p) std::vector<int32_t>();
                } else if (n.value_type == vLongs) {
                    //auto q = 
		    new (p) std::vector<int64_t>();
                } else if (n.value_type == vFloats) {
                    //auto q = 
		    new (p) std::vector<float>();
...
public:
    Jit() {
        ...
        regist_func<int64_t, int64_t, std::string, Combine>(
                [](const int64_t& a, const int64_t& b) -> std::string { return std::to_string(a) + std::to_string(b); });
        regist_func<int64_t, std::string, std::string, Combine>(
                [](const int64_t& a, const std::string& b) -> std::string { return std::to_string(a) + b; });
